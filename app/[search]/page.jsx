...
export default function Page({params}) {
    const [par, setPar] = React.useState(params.search)
    const [data, setData] = React.useState([])
    const [currentIndex, setCurrentIndex] = React.useState(0)
    const [bool, setBool] = React.useState(true)

    React.useEffect(() => {
        axios.get(`https://newsdata.io/api/1/news?apikey=pub_19255b7730c22202bce0660ecda04bdadd04b&q=${par}&country=us`)
            .then((res) => {
                        ...
                        <div className="article-image-section article-section" >
                            {value.image_url !== null ?
                                ...
                                <Image
                                    src= {newsImage}
                                    alt=""
                                    className="news-image-stock" style={bool ? mountedStyle : unmountedStyle}/>}
                        </div>
                        :
